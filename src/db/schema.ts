...
export type ReviewerAttributionSource = 'authenticated_activitypub_actor';
...
export interface ReviewDoc { id: string; title?: string; content: string; editionId: string; accountId: string; reviewerAccountId?: string; reviewerAttributionSource?: ReviewerAttributionSource; rating?: number; published: string; importedAt: string; updatedAt: string; }
...
export interface SearchIndexDependencyDoc { id: string; authorId: string; entityId: string; entityType: SearchIndexDependencyEntityType; updatedAt: string; }
...
const reviewerAttributionSource = { type: 'string', enum: ['authenticated_activitypub_actor'] } as const;
...
export const collections = {
  ...
  works: { schema: { title: 'works schema', version, type: 'object', primaryKey: 'id', additionalProperties: false, indexes: ['updatedAt'], properties: { id, title: text, summary: longText, authorIds: idList, url, importedAt: timestamp, updatedAt: timestamp }, required: ['id', 'title', 'authorIds', 'importedAt', 'updatedAt'] }, migrationStrategies: { 1: passThrough } },
  editions: { schema: { title: 'editions schema', version, type: 'object', primaryKey: 'id', additionalProperties: false, indexes: ['sourceUrl', 'updatedAt'], properties: { id, title: text, subtitle: text, description: longText, authorIds: idList, workId: id, coverUrl: url, isbn10: shortText, isbn13: shortText, sourceUrl: url, importedAt: timestamp, updatedAt: timestamp }, required: ['id', 'title', 'authorIds', 'sourceUrl', 'importedAt', 'updatedAt'] }, migrationStrategies: { 1: passThrough } },
  reviews: { schema: { title: 'reviews schema', version: reviewVersion, type: 'object', primaryKey: 'id', additionalProperties: false, indexes: ['editionId', 'accountId', 'published'], properties: { id, title: text, content: longText, editionId: id, accountId: id, reviewerAccountId: id, reviewerAttributionSource, rating: { type: 'number', minimum: 0, maximum: 5 }, published: timestamp, importedAt: timestamp, updatedAt: timestamp }, required: ['id', 'content', 'editionId', 'accountId', 'published', 'importedAt', 'updatedAt'] }, migrationStrategies: { 1: passThrough, 2: passThrough, 3: passThrough } },
  ...
  searchindexdependencies: { schema: { title: 'search index dependencies schema', version, type: 'object', primaryKey: 'id', additionalProperties: false, indexes: ['authorId', 'entityId', 'entityType', 'updatedAt'], properties: { id, authorId: id, entityId: id, entityType: searchIndexDependencyEntityType, updatedAt: timestamp }, required: ['id', 'authorId', 'entityId', 'entityType', 'updatedAt'] }, migrationStrategies: { 1: passThrough } },
