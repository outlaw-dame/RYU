...
export function enqueueModerationAction(ownerAccountId: string, action: ModerationProxyAction): ModerationQueueItem[] {
  ...
  withoutConflict.push({
    id: `${now.toString(36)}-${cryptoSafeId()}`,
...
export function actionKey(action: ModerationProxyAction): string {
  switch (action.kind) {
    ...
    case "filter_create": return `filter-local:${action.keyword.toLocaleLowerCase()}`;
    case "filter_delete": return `filter-remote:${action.filterId}`;
  }
}
...
function save(owner: string, queue: readonly ModerationQueueItem[]): void {
  ...
  try { localStorage.setItem(storageKey(owner), JSON.stringify(queue)); } catch { /* local state remains authoritative */ }
}
...
function isValidItem(value: unknown, owner: string): value is ModerationQueueItem {
  ...
  return item.ownerAccountId === owner && typeof item.id === "string" && item.id.length <= 128 &&
    typeof item.createdAt === "string" && Number.isInteger(item.attempts) && (item.attempts ?? -1) >= 0 &&
    typeof item.nextAttemptAt === "number" && Number.isFinite(item.nextAttemptAt) && isValidAction(item.action);
}
...
function isValidAction(value: unknown): value is ModerationProxyAction {
  ...
  if (kind === "filter_delete") return bounded(action.filterId, 128);
  if (kind === "filter_create") return bounded(action.title, 200) && bounded(action.keyword, 200) && Array.isArray(action.context) && action.context.length <= 5;
  return false;
}
